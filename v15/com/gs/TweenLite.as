	...
	public class TweenLite {
		...
		public function initTweenVals($hrp:Boolean = false, $reservedProps:String = ""):void {
			...
			if (this.target is Array) {
				...
				if (!isNaN(this.vars.volume) && (_isDisplayObject || this.target is SoundChannel)) { //If we're trying to change the volume of a MovieClip or Sound object, then set up a quasai proxy using an instance of a TweenLite to control the volume.
					addSubTween(volumeProxy, this.target.soundTransform, {volume:this.vars.volume}, {target:this.target});
				}
				
				for (p in v) {
					if (p == "ease" || p == "delay" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "runBackwards" || p == "persist" || p == "onUpdate" || p == "onUpdateParams" || p == "volume" || p == "onStart" || p == "onStartParams" ||p == "renderOnStart" || p == "proxiedEase" || p == "easeParams" || ($hrp && $reservedProps.indexOf(" " + p + " ") != -1)) { 
						
					} else if (!(_isDisplayObject && (p == "visible" || p == "autoAlpha" || p == "tint" || p == "removeTint" || p == "frame"))) {
						//if (this.target.hasOwnProperty(p)) { //REMOVED because there's a bug in Flash Player 10 (Beta) that incorrectly reports that DisplayObjects don't have a "z" property. This check wasn't entirely necessary anyway - it just prevented runtime errors if/when developers tried tweening properties that didn't exist.
							if (typeof(v[p]) == "number") {
								this.tweens.push({o:this.target, p:p, s:this.target[p], c:v[p] - this.target[p]}); //o:object, p:property, s:starting value, c:change in value
							} else {
								this.tweens.push({o:this.target, p:p, s:this.target[p], c:Number(v[p])}); //o:object, p:property, s:starting value, c:change in value
							}
						//}
					}
				}
